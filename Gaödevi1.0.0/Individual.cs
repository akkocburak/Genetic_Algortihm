...
namespace Gaödevi1._0._0
{
    ...
    public class Birey
    {
        ...
        {
            double terim1 = Math.Pow(Math.Sin(3 * Math.PI * X), 2);
            double terim2 = Math.Pow(X - 1, 2) * (1 + Math.Pow(Math.Sin(3 * Math.PI * Y), 2));
            double terim3 = Math.Pow(Y - 1, 2) * (1 + Math.Pow(Math.Sin(2 * Math.PI * Y), 2));
            Uygunluk = terim1 + terim2 + terim3; // Amaç fonksiyonu, numaramın sonu 9 olduğu için bu fonksiyonu kullandım.
        }

        // Rastgele birey üretme fonksiyonu -10 < x, y < 10 aralığında
        public static Birey RastgeleBirey()
        {
            double x = rastgele.NextDouble() * 20 - 10; // -10 ile 10 arasında rastgele x
            double y = rastgele.NextDouble() * 20 - 10; // -10 ile 10 arasında rastgele y
